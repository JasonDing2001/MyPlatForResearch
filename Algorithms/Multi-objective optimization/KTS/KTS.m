...
% Email:zssong@stu.xidian.edu.cn
    ...
    methods
        function main(Algorithm,Problem)
            ...
            % 更新当前存档CA，根据Q指标计算适应度，然后删除Q较小的解
            CA = UpdateCA([],Population,CAsize);
            % 更新多样性存档DA，保留非支配解，同时优先保留极端解
            ...
            % 构建代理模型,为DA+CA+P2构建一个大的代理模型？但并未使用？
            ...
            % A1是初始种群Population。
            % CA是根据Population更新的收敛性存档，DA同样，是多样性存档，DA1
            while Algorithm.NotTerminated(A1)
               % 1. 计算 Q 指标（收敛性贡献）
                Q          = Cal_Q(A1.objs); 
                [Q, index] = sort(Q,'descend'); % 将解按 Q 值从大到小排序
                
                % 2. 计算 CV（约束违反度）并对齐
                CV         = sum(max(A1.cons,0),2); 
                CV         = CV(index); % 按 Q 的排序顺序重新排列 CV
                
                % 3. 截取窗口并计算相关系数 r_coef
                ...
                % KCCMO以DA，CA，P2构建代理模型？，这里只针对约束构建代理模型。
                % 这里DA是P1，CA是？，P2是P2
                if search_mode == 1
