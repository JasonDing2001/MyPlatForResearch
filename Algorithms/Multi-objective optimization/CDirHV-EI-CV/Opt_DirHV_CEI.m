function new_x = Opt_DirHV_CEI(M,D,xlower,xupper,GPModels,CGPModels,train_y_nds,Batch_size)
...
% https://github.com/mobo-d/DirHV-EGO
   ...
   %% Use MOEA/D-GR to maximize DirHV-EI over a set of direction vectors in a collaborative manner
    ...
    for j = 1 : size(candidate_x,1)
        ...
        LFP = Feasible_Probability(repmat(candidata_con(j,:),size(dir_vecs,1),1),repmat(candidata_conMSE(j,:),size(dir_vecs,1),1));
        DirHVEIs(j,:) = DirHVEIs(j,:) .* LFP';
    end
...
%% Algorithm 2: using MOEA/D-GR to solve subproblems
    ...
    pop_LFP = Feasible_Probability(pop_con, pop_MSE);
    pop_dirhvei = pop_dirhvei .* pop_LFP;
	% optimization
    for gen = 1 : maxIter-1
       for i = 1 : pop_size    
           if rand < 0.8 % delta
           ...
           % Find the most appropriate subproblem and its neighborhood
            ...
            LFP = Feasible_Probability(off_con, off_MSE);
            DirHVEIs = DirHVEIs .* LFP;
