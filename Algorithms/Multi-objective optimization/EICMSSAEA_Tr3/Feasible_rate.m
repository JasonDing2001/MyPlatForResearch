function [priority,Feasible_rate] = Feasible_rate(Population)
...
%--------------------------------------------------------------------------
    ...
    priority = temp;  
    PopCV = sum(max(Population.cons,0),2);
    Feasible_rate1 = length(find(PopCV<=0))/size(Population,2);
    Feasible_rate = [Feasible_rate1,Feasible_rate];
end