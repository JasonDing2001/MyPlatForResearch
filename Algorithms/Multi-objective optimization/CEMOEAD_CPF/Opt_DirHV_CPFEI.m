function new_x = Opt_DirHV_CPFEI(M,D,xlower,xupper,GPModels,CGPModels,train_y_nds,Batch_size)
...
% https://github.com/mobo-d/DirHV-EGO
   ...
   %% Use MOEA/D-GR to maximize DirHV-EI over a set of direction vectors in a collaborative manner
    [~,candidate_x,candidate_mean,candidata_std,candidata_con,candidata_conMSE] = MOEAD_GR_(D,xlower,xupper,GPModels,CGPModels,dir_vecs,xis);
    ...
    candidata_CV = sum(max(candidata_con,0),2); 
    %% find q solutions with the greedy algorithm
    Qb    = subset_selection(DirHVEIs,candidata_CV,Batch_size);  
    new_x = candidate_x(Qb,:); 
end

% >>>>>>>>>>>>>>>>   Algorithm 2 & Algorithm 3  ====================
function [pop_dirhvei,pop_x,pop_mean,pop_std,pop_con,pop_MSE] = MOEAD_GR_(D,xlower,xupper,GPModels,CGPModels ,dir_vecs,xis)
%% Algorithm 2: using MOEA/D-GR to solve subproblems
	...
	[pop_con,pop_MSE] = GPEvaluate(pop_x,CGPModels);
    pop_CV = sum(max(pop_con,0),2); % the number of violated constraints
	% optimization
    for gen = 1 : maxIter-1
       for i = 1 : pop_size    
           if rand < 0.8 % delta
           ...
           [off_mean,off_std] = GPEvaluate(off_x,GPModels);  
            [off_con, off_MSE] = GPEvaluate(off_x,CGPModels);
            off_CV = sum(max(off_con,0),2);
           ...
           % Find the most appropriate subproblem and its neighborhood
            ...
            cv_less = (off_CV < pop_CV(P));
            cv_equal = (off_CV == pop_CV(P));
            dirhvei_greater = (DirHVEIs(P) > pop_dirhvei(P));
            combined_condition = cv_less | (cv_equal & dirhvei_greater);
            offindex = P(combined_condition);
           if ~isempty(offindex)
               ...
               pop_con(offindex,:)    = repmat(off_con,length(offindex),1); % popCon: N*M
               pop_MSE(offindex,:)    = repmat(off_MSE,length(offindex),1); % conMSE: N*M
               pop_CV(offindex) =  off_CV; % 更新约束违反值
            end
       end   
    end
end
...
function Qb = subset_selection(DirHVEIs, CV, Batch_size)
    %% 两阶段选择：1.优先选可行解 2.补充选CV最小的不可行解
        [L, N] = size(DirHVEIs);
        Qb = [];
        
        % 第一阶段：筛选所有可行解（CV=0）
        feasible_indices = find(CV == 0);
        num_feasible = length(feasible_indices);
        
        % 如果可行解足够，直接进行子模选择
        if num_feasible >= Batch_size
            temp = DirHVEIs(feasible_indices, :);
            beta = zeros(1, N);
            for i = 1:Batch_size
                [~, idx] = max(sum(temp - beta, 2));
                selected = feasible_indices(idx);
                Qb = [Qb, selected];
                beta = beta + temp(idx, :);
                temp = DirHVEIs(feasible_indices, :) - repmat(beta, num_feasible, 1);
                temp(temp < 0) = 0;
            end
        else
            % 第二阶段：先选所有可行解，再补充不可行解
            if num_feasible > 0
                Qb = feasible_indices; % 先加入所有可行解
                remaining = Batch_size - num_feasible;
            else
                remaining = Batch_size;
            end
            
            % 从不可行解中选择CV最小且DirHVEI高的解
            infeasible_indices = find(CV > 0);
            [~, sorted_CV] = sort(CV(infeasible_indices)); % 按CV升序排序
            infeasible_sorted = infeasible_indices(sorted_CV);
            
            % 在CV最小的前k个不可行解中，用子模选择补充剩余数量
            k = min(2*remaining, length(infeasible_sorted)); % 扩大候选池避免过早收敛
            candidates = infeasible_sorted(1:k);
            temp = DirHVEIs(candidates, :);
            beta = zeros(1, N);
            ...
            for i = 1:remaining
                ...
                Qb = [Qb, selected];
                beta = beta + temp(idx, :);
                temp = DirHVEIs(candidates, :) - repmat(beta, k, 1);
                temp(temp < 0) = 0;
            end
        end
    end
...
function LFP = Feasible_Probability(PopCon, ConMSE)
    ...
    for i = 1:N
        for j = 1:M
            P_j = normcdf(-PopCon(i,j) / sqrt(ConMSE(i,j))); % ????
            LFP(i) = LFP(i) * P_j; % ????????
        end
    end
end